...
std::pair<std::complex<double>, std::complex<double>> mband::lcalc_sampled_sigma(AmiGraph::graph_t &gself, std::vector<AmiBase::epsilon_t>& Epsilon, std::vector<AmiBase::alpha_t>& Alpha, std::vector<int>& Species,
    NewAmiCalc::ext_vars& ext_params,int MC_num, mband::params_param& param) {
    AmiGraph g(AmiBase::Sigma, 0);
    AmiBase ami;
    int ord = g.graph_order(gself);
    double prefactor = g.get_prefactor(gself, ord);
    ...
    for (int i = 0; i < MC_num; i++) {
        ...
        AmiBase::ami_vars external(energy_t, frequency, ext_params.BETA_);
        std::complex<double> result = prefactor * ami.evaluate(test_amiparms, R_array, P_array, S_array, external);
        localSum += result;
